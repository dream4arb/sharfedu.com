...
│   ├── pages/               # Page components (Home, Features, PrivacyPolicy, Lesson, Stage, Dashboard, Admin, Auth)
...
### Sitemap & Robots.txt
- **Dynamic generation**: `GET /sitemap.xml` and `GET /robots.txt` routes in `server/routes.ts` generate content dynamically from CMS hierarchy (stages, subjects, lessons).
- **Static files for Nginx**: On Cloudways, Nginx serves static files before Node.js. The admin "توليد ونشر" button (`POST /api/admin/generate-sitemap`) writes `sitemap.xml` and `robots.txt` to both `public_html/` and `node_app/server/public/`.
- **Admin panel**: Sitemap management tab under "إعدادات SEO" → "خريطة الموقع" shows URL counts, copy links, and expandable page lists.
- **Auto-update**: When new lessons/subjects are added to CMS hierarchy, admin clicks "توليد ونشر" to regenerate. The sitemap includes all static pages, stages, subjects, and individual lessons (~255 URLs).
- **Google Search Console**: Submit `https://sharfedu.com/sitemap.xml` in Google Search Console.
