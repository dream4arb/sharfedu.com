...
\n
---
⚠️ **تنبيه برمجى وهندسي عاجل (إعادة توليد بسبب فشل الرسومات):**
لقد فشلت المحاولات السابقة في تقديم رسومات علمية صحيحة. المستخدم يرى علامات "+" و "-" مبعثرة وغير مفهومة (مثل +++++++).
**المطلوب فوراً وبدقة متناهية:**
1. **الرسومات العلمية (SVG):** لا تضع علامات نصية (+ أو -) بشكل عشوائي. إذا كنت ترسم "شحنات كهربائية" في سحابة أو أرض، ارسمها كرموز فنية داخل دوائر صغيرة أو بتوزيع هندسي متناسق ومنظم جداً، وليس مجرد نص متكرر.
2. **البرق والسحب:** ارسم السحب كأشكال هندسية ناعمة (Paths) والبرق كخط متعرج احترافي (Stroke) متصل ومنسق.
3. **التداخل الهيكلي:** يمنع تماماً تداخل النصوص مع الرسومات. يجب أن يكون لكل عنصر مساحته الخاصة (Padding/Margin).
4. **جودة الكود:** استخدم CSS Grid/Flexbox لضمان التناسق. لا تستخدم position:absolute بشكل يعطل التجاوب أو يسبب تداخل العناصر.
5. **الالتزام بالقواعد:** راجع ملف قواعد SVG المرفق وطبقه حرفياً، خاصة القاعدة التي تمنع النصوص العشوائية داخل الرسومات.
---
`;
    }
